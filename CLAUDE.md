...
### Timed-Input Must Auto-Submit (never discard typed/selected input on timeout)
When a timed phase collects input the CLIENT holds locally (a typed draft, a slider value, a chosen order, accumulated bets) and only sends on a Submit/Lock button, the timer expiring MUST auto-submit whatever is entered — otherwise the player's work is silently lost (or replaced by a server house-value). **Client pattern** (gold standard `client/src/games/Scattergories.jsx`): keep a `xRef` mirroring the latest local input (`xRef.current = x` every render), an `autoSubmitted` ref reset on phase entry, and in the countdown tick fire the SAME submit action ~1s BEFORE the server deadline (`if (rem <= 1 && !autoSubmitted.current) { autoSubmitted.current = true; onAction({type:'submit', ...xRef.current}); }`) so it lands before the server reveals. Submit empty/partial — it must beat the server's discard/house-substitute. The server needs no change (it already accepts a normal submit). Applied across Scattergories, Timeline, Price Is Wrong, Guesstimate, Roulette, Wavelength (clue+guess), Fibbage (write+vote), Telephone Pictionary, Rank It, Definition Duel, Quiplash, Caption This, Awkward Award, Group Mind, Two Truths.

### Reveal/Results Screens: Continue button, not a short countdown
A results/reveal/showdown screen that shows everyone's answers should advance when all present players tap **Continue** (`acknowledge`), with the server's auto-advance kept only as a **long (~45–60s) AFK safety net with NO visible countdown** — short 4–10s auto-timers rush reading. Requires: a Continue button + `AckStatus` in the client reveal section, the engine advancing on the all-acked barrier, and a lengthened `REVEAL_MS`. If the client also shows a countdown bound to the phase deadline, gate it to the INPUT phase only so the long reveal safety timer stays invisible (see Timeline's `useCountdown(phase==='guessing' ? deadline : null)`). Applied to Scattergories, Poker (showdown had a 4s no-Continue auto-advance), Timeline, Price Is Wrong, Guesstimate, Higher or Lower. **When lengthening REVEAL_MS, update the test that does `advance(<old REVEAL_MS>)` for the reveal-timeout case.**

### Roulette Betting Deadlock (fixed)
Roulette's betting phase originally had NO server timer — it only spun once every non-broke player submitted, so one AFK player stalled the round forever. It now arms a betting-phase timer (`TIMERS.ROULETTE`) that resolves the spin with whatever was submitted on expiry (paired with `_emitChange`, cleared on spin/destroy/leave), and exposes `betEndTime` so the client shows a countdown + auto-submits pending bets.

### Every Decision Phase Needs a Server Timer (v2.42.0 idle-player overhaul)
The orchestration layer has NO global turn/idle timeout — if a game's decision phase lacks its own timer, a *connected-but-idle* player (backgrounded tab; `removePlayer` only fires on actual disconnect) freezes the game for everyone with no recovery. A 4-agent audit found the six oldest games had exactly this gap; all now arm a per-decision timer with a sensible auto-action and expose the deadline so clients show a countdown:
- **Blackjack** — 30s/turn, auto-**stand** (fresh clock after a non-bust hit); exposes `turnEndsAt`
- **Poker** — 30s per betting decision, auto-**check** if free else auto-**fold**; exposes `turnEndsAt` (reveal already had its 45s safety)
- **Liar's Dice** — 30s/bid: no current bid → minimal bid (1×2); else minimal raise; auto-**challenge** once the raise would exceed all dice in play (guarantees termination); exposes `turnEndsAt`
- **Go Fish** — 30s/turn, auto-ask a random (rank-from-hand, other-player) pair; exposes `turnEndsAt`
- **Crazy Eights** — 30s/turn, auto-play first playable (an 8 picks the most-held suit) else draw; exposes `turnEndsAt`
- **RPS** — 15s choosing phase, random-fills non-submitters then resolves; exposes `choiceEndsAt`
Pattern (copy President.js): `_startTurnTimer()` clears the old timer, guards `state`, stores `_turnEndsAt = Date.now() + TURN_MS`, and the callback re-guards state + calls `_emitChange()`; cleared in `destroy()` and re-armed on every turn change **including the removePlayer turn-reassignment path**. Auto-actions go through `handleAction` so all validation/advancement logic is reused. Clients render a small "⏱ Ns" countdown next to the turn indicator. **Store the deadline once when arming** — recomputing `Date.now() + TURN_MS` in `getStateForPlayer` makes the client countdown reset on every unrelated broadcast (the old PressYourLuckPigs bug).

### Poker Split Pot (fixed v2.42.0)
`_resolveShowdown` originally picked a single winner with a strictly-better comparison, so an exact hand tie gave the whole pot to whoever came first in `this.players`. It now collects ALL tied best hands and distributes via investment layers: each layer is split equally among tied winners staked at that level (remainder chips to earliest winner), and layers above every winner's stake are refunded to their contributors — side-pot-correct and chip-conserving. `handResults` gained a `winners` array (kept `winner` for back-compat) and appends "(split pot)" to the description.

### Rules of Hooks: no hooks after the `!gameState` guard
Six legacy game components (Poker, GoFish, MemoryMatch, Battleship, SpotTheDifference, Blackjack) called useState/useEffect/useRef AFTER the early `if (!gameState) return` — the hook count changed between the null first render and the loaded render (React crash: "Rendered more hooks than during the previous render"). All hooks must sit above the guard; effects null-guard internally and derive inputs with `gameState?.`.
